...
class CommNetMLP(nn.Module):
    ...
    def __init__(self, args, num_inputs):
        ...
        if self.args.env_name == 'rware':
            """
            replace xxx in the last layer with the size of your ground truth
            """
            self.init_dim = 5  # Initial feature map size

            # # Experiment CNN
            # self.mapdecode = nn.Sequential(
            #     nn.Linear(args.hid_size, 32 * self.init_dim * self.init_dim),
            ...
            #     nn.Conv2d(8, 4, kernel_size=1),  # → (batch, 3, 10, 10)
            #     nn.ReLU(inplace=True),   # was nn.Sigmoid

            #     nn.AdaptiveAvgPool2d((7, 7)),  # 4x7x7 = 196
            #     nn.Flatten(),
            #     nn.Linear(196, 36)
            # )

            # Experiment MLP
            # 48 = (4 agents + 4 queued requests) * 6 features, i.e. the
            # flattened output of RwareEnv.ground_truth()
            self.mapdecode = nn.Sequential(
                nn.Linear(self.args.hid_size, self.args.hid_size),
                nn.ReLU(inplace=True),
                nn.Linear(self.args.hid_size, self.args.hid_size),
                nn.ReLU(inplace=True),
                nn.Linear(self.args.hid_size, 48)
            )
            # RWARE has no grid-shaped ground truth to decode into.
            self.cnndecode = None
        elif self.args.env_name == 'traffic_junction':
            # One node per car (there is no prey node as in predator_prey),
            # each decoded as position (2) + motion vector (2).
            self.mapdecode = nn.Sequential(
                nn.Linear(self.args.hid_size, self.args.hid_size),
                nn.Linear(self.args.hid_size, self.args.nagents * (2+2)),
                nn.LeakyReLU()
            )
            self.init_dim = 5  # Initial feature map size
            # Traffic junction tracks no exploration/obstacle grid, so there is
            # no grid-shaped ground truth for the CNN decoder to predict.
            self.cnndecode = None
        else:
            self.mapdecode = nn.Sequential(
                nn.Linear(self.args.hid_size, self.args.hid_size),
        ...
        # RWARE encoder variants kept from the RWARE branch for reference:
        #
        # self.encoder = nn.Sequential(
        #     nn.Conv2d(7, 16, (3,3)),    # 7 corresponding to the layers of input
        #     nn.ReLU(inplace=False),
        #     nn.Conv2d(16, 4, (1,1)),
        #     nn.ReLU(inplace=False),
        #     nn.Flatten(),
        #     nn.Linear((self.args.dim-3+1)*(self.args.dim-3+1)*4, args.hid_size)
        # )
        #
        # # New Encoder
        # self.encoder = nn.Sequential(
        ...
        # )
        if self.args.env_name == 'rware':
            # Old Encoder (two stacked linear layers, as on the RWARE branch)
            self.encoder = nn.Sequential(
                    nn.Linear(num_inputs, args.hid_size),
                    nn.Linear(args.hid_size, args.hid_size))
        else:
        ...
        # The rware loss has no global value term, so the head is skipped there.
        # Not building it also keeps the RNG stream identical to the RWARE
        # branch, so runs stay reproducible against it.
        if self.args.env_name == 'rware':
    ...
    def forward(self, x, info={}):
        ...
        for i in range(self.comm_passes):
            ...
            if self.args.recurrent:
                ...
                cnn = self.cnndecode(decoder_input) if self.cnndecode is not None else None
                ...
                cnn = self.cnndecode(hidden_state) if self.cnndecode is not None else None
        ...
        value_global = self.value_global(hidden_state).view(value_head.shape[0],value_head.shape[0]) \
            if self.value_global is not None else None #1
