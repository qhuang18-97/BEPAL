...
# Teacher distillation (rware)
parser.add_argument('--use_teacher', default=False, action='store_true',
                    help='use teacher or not')
...
# init_args_for_env() returns early for env names it does not know, so rware
# never gets a --dim registered on the parser.
if args.env_name == 'rware' and not hasattr(args, 'dim'):
    args.dim = 7
...
log['penalty'] = LogField(list(), True, 'epoch', 'num_episodes')
...
log['kl_loss'] = LogField(list(), True, 'epoch', 'num_steps')
...
def run(num_epochs):
    ...
    for ep in range(num_epochs):
        ...
        if 'kl_loss' in stat.keys():
            print('kl loss: {}'.format(stat['kl_loss']), flush=True)
        ...
        # Envs without a get_stat() (e.g. rware) never report success.
        if args.save != '' and stat.get('success', 0) > global_succ:
            global_succ = stat['success']
            save(args.save+'_succ_ml')


def resolve_save_path(path):
    # A leading separator means "relative to the repo root" (see memo.txt,
    # e.g. --save /gcn-results/gcn_d12_dy); anything else is taken as given,
    # so relative paths like --save results/ work from the cwd.
    ...
    # (--save results/ before the first periodic save has created it).
...
def load(path):
    d = torch.load(resolve_save_path(path))
    # log.clear()
    policy_net.load_state_dict(d['policy_net'])
    log.update(d['log'])
