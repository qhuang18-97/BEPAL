...
class TrafficJunctionEnv(gym.Env):
    ...
    def step(self, action):
        ...
        return obs, action_mask, reward, self.episode_over, debug
    ...
    def _get_obs(self):
        ...
        obs = tuple(obs)
        # env_wrappers.GymWrapper expects every env to hand back an action mask
        # alongside the observation (see predator_prey_env._get_obs). A car can
        # always GAS or BRAKE, so every action is valid at every step.
        action_mask = np.ones((self.ncar, self.naction), dtype=int)
        return obs, action_mask
